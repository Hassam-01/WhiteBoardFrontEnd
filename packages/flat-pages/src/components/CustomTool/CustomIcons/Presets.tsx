// import "../style.less";
import React from "react";
import { FlatIconProps } from "../types";

export const SVGPresets: React.FC<FlatIconProps> = ({ active, className = "", ...restProps }) => {
    return (
        <svg
            className={`${className} flat-icon ${active ? "is-active" : ""}`}
            fill="none"
            height="30"
            viewBox="0 0 31 30"
            width="31"
            xmlns="http://www.w3.org/2000/svg"
            {...restProps}
        >
            <path
                d="M6.75 26.25C6.0625 26.25 5.47396 26.0052 4.98438 25.5156C4.49479 25.026 4.25 24.4375 4.25 23.75V6.25C4.25 5.5625 4.49479 4.97396 4.98438 4.48438C5.47396 3.99479 6.0625 3.75 6.75 3.75H24.25C24.9375 3.75 25.526 3.99479 26.0156 4.48438C26.5052 4.97396 26.75 5.5625 26.75 6.25V23.75C26.75 24.4375 26.5052 25.026 26.0156 25.5156C25.526 26.0052 24.9375 26.25 24.25 26.25H6.75ZM9.25 21.25H21.75C22 21.25 22.1875 21.1354 22.3125 20.9062C22.4375 20.6771 22.4167 20.4583 22.25 20.25L18.8125 15.6563C18.6875 15.4896 18.5208 15.4062 18.3125 15.4062C18.1042 15.4062 17.9375 15.4896 17.8125 15.6563L14.5625 20L12.25 16.9063C12.125 16.7396 11.9583 16.6562 11.75 16.6562C11.5417 16.6562 11.375 16.7396 11.25 16.9063L8.75 20.25C8.58333 20.4583 8.5625 20.6771 8.6875 20.9062C8.8125 21.1354 9 21.25 9.25 21.25Z"
                fill="#6F3393"
            />
        </svg>
    );
};

export default SVGPresets;
